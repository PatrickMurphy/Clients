...
if (isset($_POST['createwebsite'])) {
        ...
        $equip = addslashes($_POST['equipment']);
        ...
        if ($error == null) {
            $query = 'UPDATE photos SET designerid=' . $designer . ', clientid=' . $client . ', title=\'' . $title . '\', url=\'' . $_POST['url'] . '\', description=\'' . $desc . '\', 
                equipment=\'' . $equip . '\', commissiondate=\'' . $commissioned . '\', deadline=\'' . $deadline .
                '\', status=\'' . $_POST['status'] . '\', finishdate=\'' . $finished . '\', price=\'' . $_POST['price'] . '\',
				 paymentrecieved=\''. $paymentrecievedvalue . '\', finished='.$finished1.', referid='.$_POST['referid'].' WHERE id = ' . $_POST['id'];
            if ($ifquery = mysql_query($query)) {
                print '<div class="success">Photos Edited</div>';
            } else {
        ...
        $photosquery = mysql_query('SELECT * FROM photos WHERE id=' . $_GET['id']);
        $web = mysql_fetch_array($photosquery);
...
?>
<form action="index.php?act=editphoto&designerid=<?php print $_SESSION['userid']; ?>" id="creatwebsite" method="post">
    ...
    <label for="title">Title:</label><input type="text" name="title" id="title" value="<?php print stripslashes($web['title']); ?>" /><input type="hidden" name="id" style="display:none" value="<?php print $_GET['id']; ?>" /><br />
    ...
    <label for="equipment">Equipment:</label><textarea id="equipment" name="equipment"><?php print stripslashes($web['equipment']);?></textarea><br />
...
$year3; ?></select><br />
    ...
    <input type="submit" class="submit" name="createwebsite" value="Edit Photo" /></form>
    <?php 
    }
    $jobid = $_GET['id'];
