...
function MyAjaxRequest(target_div,file,check_div,file2,check_div2)
{
var MyHttpRequest = false;
var MyHttpLoading = '<option>Loading...</option>'; // or use an animated gif instead: var MyHttpLoading = '<img src="loading.gif" border="0" alt="running" />';
...
else // we have presumable a php file with a query string attached
{
  ...
  var query_string2 = check_value2 + '&rand=' + random;
}


MyHttpRequest.open("get", url_encode(file + query_string + file2 + query_string2), true); // <-- run the httprequest using GET
